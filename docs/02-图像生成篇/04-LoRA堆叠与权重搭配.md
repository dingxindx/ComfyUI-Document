...
| 方式 | 说明 | 例子 |
|:-----|:------|:-----|
| **单 LoRA 使用** | 加载一个 LoRA 即可 | 给 SD 模型增加一个角色的特征 |
| **多 LoRA 堆叠** | 串联多个 LoRA 叠加效果 | 同时加载角色 + 服装 + 背景风格 |
| **LoRA + 其他技术** | 与 ControlNet / IP-Adapter 等共存 | LoRA + ControlNet 不冲突 |

> 💡 想知道 LoRA 与其他技术的区别和选型？→ [技术选型与组合参考](00-技术选型与组合参考.md)
...
> 💡 LoRA 修改 model（紫色），ControlNet 修改 conditioning（橙色），IP-Adapter 也修改 model（紫色但作用于不同维度）→ 它们作用于不同数据流，**互不冲突**。各组合的接线方案详见 [技术选型与组合参考 §三](00-技术选型与组合参考.md#三组合共存接线方案)。

### 5.1 LoRA + ControlNet 共存

LoRA 在 model 流上串联，ControlNet 在 conditioning 流上并联。两个维度互不干扰。

### 5.2 LoRA + IP-Adapter 共存

**顺序很重要**：LoRA 在先（建立角色特征），IP-Adapter 在后（注入风格）。如果反过来，LoRA 会覆盖掉 IP-Adapter 的风格。
