...
**用终端**：
```bash
# 假设下载文件在 ~/Downloads/dreamshaper_8.safetensors
cp ~/Downloads/dreamshaper_8.safetensors ~/workspace/ComfyUI/models/checkpoints/
```

**用访达（Finder）**：
1. 打开访达
2. 前往 `~/workspace/ComfyUI/models/checkpoints/`
3. 把下载的 `.safetensors` 文件拖进去

> ✅ 验证：在终端输入 `ls ~/workspace/ComfyUI/models/checkpoints/`，看到你的模型文件名。
...
```
CheckpointLoaderSimple          (根节点，不需要输入)
  │
  ├── MODEL ──────→ KSampler.model               (紫色连紫色)
  ├── CLIP ────────→ CLIP Text Encode (Prompt).clip   (粉色连粉色)
  │                 → CLIP Text Encode (Negative).clip (粉色连粉色)
  └── VAE ────────→ VAE Decode.vae               (黄色连黄色)

CLIP Text Encode (Prompt)  ──→ KSampler.positive     (橙色连橙色)
CLIP Text Encode (Negative) ──→ KSampler.negative     (橙色连橙色)

Empty Latent Image ──→ KSampler.latent_image    (蓝色连蓝色)

KSampler ──→ VAE Decode.samples                  (蓝色连蓝色)

VAE Decode ──→ Save Image.images                  (绿色连绿色)
```

**连线操作口诀**：
1. 从 `CheckpointLoaderSimple` 右侧的 `MODEL` 端口（紫色圆点）**按住左键拖出线**
2. 连接到 `KSampler` 左侧的 `model` 端口（紫色圆点），**松开**
3. 重复上述操作连接其他端口

> ✅ 验证：所有连接线应该是**白色的**。如果某根线变成**红色**，说明连接的数据类型不匹配，检查一下连的是不是相同颜色的端口。
